...
export function FeatureStrip() {
  return (
    <section id="how-it-works" className="relative z-10 mx-auto max-w-7xl border-y border-(--color-border) px-6 py-10 md:px-10">
      <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
        {FEATURES.map(({ icon: Icon, title, subtitle }) => (
          <div key={title} className="flex items-center gap-3">
            <span className="flex size-11 shrink-0 items-center justify-center rounded-full border border-(--color-border) text-(--color-sage)">
              <Icon size={20} />
            </span>
            <div>
