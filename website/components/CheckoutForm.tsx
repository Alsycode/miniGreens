...
export function CheckoutForm({
  ...
  if (confirmed) {
    return (
      <div className="rounded-2xl border border-(--color-border) bg-(--color-surface) p-8 text-center">
        <CheckCircle size={44} weight="fill" className="mx-auto text-(--color-sage)" />
        <h2 className="font-display mt-5 text-3xl">{confirmationTitle}</h2>
        <p className="mt-3 text-sm leading-relaxed text-(--color-muted)">
          {confirmationBody}
        </p>
        <p className="mt-6 text-sm text-(--color-muted)">
          We&apos;ll reach {details.fullName} on {details.phone} to confirm the
          delivery on {details.deliveryDate} between {details.deliveryTime}.
        </p>
        <Link
          href="/"
          className="mt-8 inline-flex rounded-full bg-(--color-olive) px-7 py-3.5 text-sm font-medium text-white transition-colors hover:bg-(--color-olive-dark)"
        >
          Back to home
        </Link>
      </div>
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        className="mt-7 w-full rounded-full bg-(--color-olive) px-7 py-4 font-medium text-white transition-colors hover:bg-(--color-olive-dark)"
      >
        {submitLabel}
      </button>
