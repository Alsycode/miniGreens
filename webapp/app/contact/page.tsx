...
export default function ContactPage() {
  return (
    <PageShell
      eyebrow="Contact"
      title="Talk To The People Who Grow It"
          ...
          <ul className="space-y-5">
            {DETAILS.map(({ icon: Icon, label, value }) => (
              <li key={label} className="flex items-start gap-3">
                <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-(--color-navy)/10 text-(--color-navy)">
                  <Icon size={16} />
                </span>
                <div>
                  ...
                  <p className="mt-1 text-(--color-ink)">{value}</p>
                </div>
              </li>
            ))}
          </ul>
